...
public class TelInfoDAO {
	...
	public boolean insert_nametel(int id, String name, String tel, String d){
		String sql = "insert into TelTable values(?,?,?,TO_DATE(?,'YYYY-MM-DD'))";
		try{
	...
	public TelInfoVO search_nametel(String oriName)//홍길동
	                              		throws SQLException{
		TelInfoVO tv=null;
		String sql = "SELECT * FROM TelTable where name=?";
		//? : 원하는 사람 이름 
		pstmt = con.prepareStatement(sql);
	...
	public boolean update_nametel(int id1, String name1, String tel1,String d1,String name2){
		    //name->tel2  
		String sql = "update TelTable set id=?,name=?,tel=?,D=TO_DATE(?,'YYYY-MM-DD') where name=?";
		try{
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1, id1);
	...
	public boolean delete_nametel(String name2){
		String sql = "delete from TelTable where name=?";/////
		try{
			pstmt=con.prepareStatement(sql);
